...
final navigationControllerProvider =
    Provider((ref) => NavigationController(ref: ref));
...
class NavigationController {
  ...
  void addNewRequest() {
    _ref.read(requestListProvider.notifier).update(
      (state) {
    ...
    _ref.read(selectedRequestIndexProvider.notifier).update(
      (state) {
    ...
    _ref.read(requestListProvider.notifier).update(
      (state) {
        ...
        if (state.length - 1 >= index) {
          ...
          _ref
              .read(selectedRequestIndexProvider.notifier)
